...
resizeImage.get("/", async (req: Request, res: Response) => {
  ...
  if (!filename || !height || !width) {
    res
      .status(400)
      .send(
        "Please enter a valid URL containg the filename, height and the width parameters :)"
      );
    return;
  }
  ...
  const cashImagePath = `${path.resolve(
    __dirname,
    `../../../Images/cashes/${filename}.jpeg`
  )}`;

  const fullImage: Stats | null = await fs.promises
    .stat(fullImagePath)
    .catch(() => {
  ...
  const cashImage: Stats | null = await fs.promises
    .stat(cashImagePath)
    .catch(() => {
      return null;
    });
  ...
  if (cashImage) {
    fs.promises.readFile(cashImagePath).then((data) => {
      res.status(200).contentType("jpeg").send(data);
    });
  } else {
    const data: Buffer | null = await fs.promises
    ...
    fs.promises
      .writeFile(
        `${path.resolve(
          __dirname,
          `../../../Images/cashes/${filename}${width}_${height}.jpeg`
        )}`,
        resizedImage
      )
      .then(() => {
        res.status(200).contentType("jpeg").send(resizedImage);
      })
      .catch(() => {
